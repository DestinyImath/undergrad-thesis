...
def get_transforms(use_augmentation=True):
    ...
    新增 use_augmentation 开关，用于消融实验。
    """
    # --- 基础预处理流水线 (无论是训练集还是验证集，无论是否开启增强，都必须执行) ---
    base_transforms = [
        ...
        Resized(keys=["image", "label"], spatial_size=(256, 256)) # 统一缩放到 256x256
    ]
    
    # --- 动态构建训练集 Transforms ---
    if use_augmentation:
        # 如果开启增强，将增强组件追加到基础预处理之后
    ...
    # 验证集：永远只做预处理，绝对不能做数据增强！
    val_transforms = Compose(base_transforms)
    
    return train_transforms, val_transforms

def get_dataloaders(data_dir, batch_size=8, num_workers=4, use_augmentation=True):
    """
    步骤 3: 封装成 PyTorch 可用的 DataLoader
    将 use_augmentation 开关暴露给外部调用者。
    """
    train_files, val_files = get_busi_data_dicts(data_dir)
    train_transforms, val_transforms = get_transforms(use_augmentation=use_augmentation)
    
    # 打印当前的数据增强状态，方便跑实验时确认记录
    aug_status = "✅ 已开启" if use_augmentation else "❌ 已关闭 (消融实验模式)"
...
if __name__ == "__main__":
    ...
    # 测试关闭增强 (消融实验)
    print("\n--- 测试 2: 关闭增强 (Ablation) ---")
    train_loader_no_aug, val_loader_no_aug = get_dataloaders(DATA_DIR, batch_size=4, num_workers=0, use_augmentation=False)
