...
from function_lib import ResNetBackbone, UniversalDeepLabV3Plus, CustomDiceCELoss
...
# --- 模型动态命名与路径管理 ---
MODEL_NAME = "deeplab_res50+no_aspp"
...
# 想要跑 ResNet50 + DeepLabV3+
backbone = ResNetBackbone(model_name="resnet50")
model = UniversalDeepLabV3Plus(
    backbone=backbone,
    low_level_channels=256,   # ResNet Layer1 输出是 256
    high_level_channels=1024, # ResNet Layer3 输出是 1024
    out_channels=1,
    use_aspp=False,
    use_decoder=True
).to(device)
